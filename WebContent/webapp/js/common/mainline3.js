...
function TMainLine(afrmOptions) {
    ...
    var detailGrids = (afrmOptions.detailGrids == undefined) ? [] : afrmOptions.detailGrids;//新的行列表
    ...
    initdetailGrids();  //初始化明细行参数
    function initdetailGrids() {
        //alert(detailGrids.length);
        for (var k = 0; k < detailGrids.length; k++) {
            var dop = detailGrids[k];
            dop.grdid = (dop.grdid) ? dop.grdid : "extglid" + $generateUUID();
            dop.lineHeight = (dop.lineHeight) ? dop.lineHeight : "300px";
            dop.lineTitle = (dop.lineTitle) ? dop.lineTitle : "明细数据";
            if (!dop.lineColumns) {
                alert("detailGrids 每项都需要 lineColumns");
            }
            var lineColumns = dop.lineColumns;
            if (lineColumns) {//兼容两种列定义方式
                var col1 = lineColumns[0];
                lineColumns = ($.isArray(col1)) ? lineColumns : [lineColumns];
                for (var i = 0; i < lineColumns.length; i++) {
                    var col = lineColumns[i];
                    if ($.isArray(col)) {
    ...
    function initUI() {
        ...
        function doinitext() {
            ...
            if (wfform) {//流程表单
                ...
                if (htmlTemeType == HtmlTempType.htMLPop) {
                    ...
                    $("#datainfo_window_id").window({closable: false, maximizable: false, draggable: false});
                }
            } else {
                if ((istree) || (autoFind))
                    do_find([]);
        ...
        function initGrids() {
            ...
            for (var i = 0; i < detailGrids.length; i++) {
                var dop = detailGrids[i];
                var hml = "  <div class='datainfo_title'> " + dop.lineTitle + "</div><div style='height: " + dop.lineHeight + "'><table id='" + dop.grdid
                    + "' style='width: 100%;height: 100%'></table></div>";
                $(hml).appendTo("#detail_main_grid_div");
            }

            $.parser.parse('#detail_main_grid_div');
            for (var i = 0; i < detailGrids.length; i++) {
                var dop = detailGrids[i];
                var go = $("#" + dop.grdid);
                var grdoptions = {
                    onClickRow: function (index, row) {
                        var gid = $(this).parent().find("table[id]").attr("id");
                    ...
                    onDblClickRow: function (index, row) {
                        if (caneditlinedata || isCanEdit) {
                            var gid = $(this).parent().find("table[id]").attr("id");
                            var tdop = getDopByGrdid(gid);
                            if (tdop.onDBClick) {
                                if (tdop.onDBClick(index, row)) {
                                    $C.grid.edit(gid);
                                    if (tdop.onBeginEdit())
                                        tdop.onBeginEdit(index, row);
                                }
                            } else {
                                $C.grid.edit(gid);
                                if (tdop.onBeginEdit)
                    ...
                    onAfterEdit: function (index, row, changes) {
                        ...
                        if (tdop.onAfterEdit)
                            tdop.onAfterEdit(index, row, changes);
                    },
                    border: false,
                    columns: dop.lineColumns,
                ...
                dop.toolBar = [
                    {
                        ...
                        iconCls: 'icon-add1',
                        handler: function () {
                            var gid = $(this).parents(".datagrid-toolbar").next().find("table[id]").attr("id");
                            var tdop = getDopByGrdid(gid);
                            if (caneditlinedata || isCanEdit) {
                                var row = undefined;
                                if (tdop.onLineAppend) {
                                    row = tdop.onLineAppend();
                                    if (row)
                                        $C.grid.append(tdop.grdid, row, true);
                                } else {
                                    $C.grid.append(tdop.grdid, {}, true);
                                }
                    ...
                    "-",
                    {
                        ...
                        handler: function () {
                            if (caneditlinedata || isCanEdit) {
                                var gid = $(this).parents(".datagrid-toolbar").next().find("table[id]").attr("id");
                                var tdop = getDopByGrdid(gid);
                                var rowdata = $("#" + gid).datagrid("getSelected");
                                if (!rowdata) {
                                    $.messager.alert('错误', '没有选择的数据！', 'error');
                                    return;
                                }
                                if (tdop.onRemoveLine) {
                                    if (tdop.onRemoveLine(rowdata)) {
                                        $C.grid.remove(gid);
                                    }
                                } else {
                                    $C.grid.remove(gid);
                            ...
                            if (caneditlinedata || isCanEdit) {
                                ...
                                $.messager.confirm('确认', '确认清空所有行？', function (r) {
                                    if (r) {
                                        ...
                                        if (tdop.onAfterClear)
                                            tdop.onAfterClear(gid);
                ...
                if (dop.extToolBar)
                    dop.toolBar = dop.toolBar.concat(dop.extToolBar);

                if (dop.options) {
                    grdoptions = $.extend(grdoptions, $.c_parseOptionsStr(dop.options));
                }
                go.datagrid(grdoptions);
            }//end for
    ...
    function reflashUI(stat) {
        ...
        for (var i = 0; i < detailGrids.length; i++) {
            ...
            if ((caneditlinedata || isCanEdit) && (!disableLineButtons)) {
                var onSetToolbar = (dop.onSetToolbar) ? dop.onSetToolbar : undefined;
                var tb = (onSetToolbar) ? onSetToolbar(dop.grdid, dop.toolBar) : dop.toolBar;
                $("#" + dop.grdid).datagrid({toolbar: tb});
            } else {
                $("#" + dop.grdid).datagrid({toolbar: []});
            }
        }
        reSetBts();//设置按钮enable
        reSetInput();//设置输入框
...
//Options:content_filter;content;  tabid;title;closable;
    ...
    function clearViewData() {
        var gids = getlinegridids();
        JSONBandingFrm.clearMainData(els_all, gids);
    }

    this.clearGridViewData = clearGridViewData;
    function clearGridViewData() {
        var gids = getlinegridids();
        JSONBandingFrm.clearGridViewData(gids);
    }
...
//save
    ...
    function dosave(andSubmit, isclicksave) {
        ...
        if (formtype == TFrmType.ftMainLine) {
            ...
            for (var i = 0; i < detailGrids.length; i++) {
                var dop = detailGrids[i];
                gls.push(dop.lineProperty);
                glcs.push(dop.lineColumns);
            }
            if (!JSONBandingFrm.checkNotNull(els_all, lables, curMainData, gls, glcs)) return false;
        } else {
        ...
        for (var i = 0; i < detailGrids.length; i++) {
            var dop = detailGrids[i];
            if (dop.allowEmptyLine == false) {
                var lds = curMainData[dop.lineProperty];
                if ((!lds) || (lds.length == 0)) {
                    $.messager.alert('错误', '【' + dop.lineTitle + '】不允许为空！', 'error');
                    return;
                }
            }
        }
        ...
        postData2Save(postData, andSubmit, isclicksave);
    }
    ...
    function postData2Save(postData, andSubmit, isclicksave) {
...
////find begin
    ...
    function setJson2Grids() {
        if (formtype == TFrmType.ftMainLine) {
            for (var i = 0; i < detailGrids.length; i++) {
                try {
                    var dop = detailGrids[i];
                    if (!curMainData[dop.lineProperty])
                        curMainData[dop.lineProperty] = [];
                    $("#" + dop.grdid).datagrid("loadData", curMainData[dop.lineProperty]);
                } catch (e) {
                    console.error(e);
                }
            }
            ...
            if (co.caction == "act_ok") {
                el.onclick = function () {
                    ...
                    if (afrmOptions.onExport) {
                        var wfcls = afrmOptions.onExport(expflds);
    ...
    function doexportlst1() {
        ...
        _perparms.cols = fcls;
        if (afrmOptions.onExport) {
            var wfcls = afrmOptions.onExport(fcls);
            if (wfcls)_perparms.cols = wfcls;
        }
...
//public proterty
    ...
    function getMainData() {
        ...
        for (var i = 0; i < detailGrids.length; i++) {
            var dop = detailGrids[i];
            $C.grid.accept(dop.grdid);
            curMainData[dop.lineProperty] = $("#" + dop.grdid).datagrid("getRows");
        }
    ...
    function getSelectedLine(filter) {
        return $(filter).datagrid("getSelected");
    }
...
//other functions
     ...
     */
    function onValueChange(fdname, newValue, oldValue) {
        if (oldValue != newValue) {
            if ((afrmOptions.onEditChanged) && (!isloadingdata)) {//&& (!issetEditValue)
                afrmOptions.onEditChanged(fdname, newValue, oldValue);
            }
