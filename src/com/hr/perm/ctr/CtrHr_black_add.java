...
public class CtrHr_black_add extends JPAController {
	@Override
	...
	private void doadd(CJPA jpa, CDBConnection con) throws Exception {
		...
		if (ba.billtype.getAsFloatDefault(0) == 1) {
			...
			if (lv.iscanced.getAsInt() == 1)
				throw new Exception("离职表单已经撤销，不允许加封黑名单！");
			// lv.isblacklist.setAsInt(1); 不能设置，否则解封后 不能再次加封
		}
		emp.empstatid.setAsInt(13);
		// lv.save(con);
		emp.save(con);
		CtrHrEmployeeUtil.addBlackList(con, emp.id_number.getValue());
	}
