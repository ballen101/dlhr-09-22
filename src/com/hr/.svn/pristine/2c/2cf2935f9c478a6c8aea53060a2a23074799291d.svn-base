...
@CEntity(caption="薪资异动报批",tablename="Hr_salary_chglgsubmit")
public class Hr_salary_chglgsubmit extends CJPA {
	@CFieldinfo(fieldname ="cls_id",iskey=true,notnull=true,precision=20,scale=0,caption="异动记录报批ID",datetype=Types.INTEGER)
	public CField cls_id;  //异动记录报批ID
	@CFieldinfo(fieldname ="cls_code",notnull=true,codeid=144,precision=16,scale=0,caption="异动记录报批编码",datetype=Types.VARCHAR)
	public CField cls_code;  //异动记录报批编码
	...
	@CLinkFieldInfo(jpaclass = Hr_salary_chglgsubmit_line.class, linkFields = { @LinkFieldItem(lfield = "cls_id", mfield = "cls_id") })
	public CJPALineData<Hr_salary_chglgsubmit_line> hr_salary_chglgsubmit_lines;

	public Hr_salary_chglgsubmit() throws Exception {
	}
