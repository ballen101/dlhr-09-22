...
public class CtrHr_kq_monthsubmit extends JPAController { 

	@Override
	public void AfterWFStart(CJPA jpa, CDBConnection con, Shwwf wf, boolean isFilished) throws Exception {
		//检查当月是否有提报
		Hr_kq_monthsubmit h = (Hr_kq_monthsubmit) jpa;
		CJPALineData<Hr_kq_monthsubmit_line> ls = h.hr_kq_monthsubmit_lines;
		for (CJPABase j : ls) {
			Hr_kq_monthsubmit_line l = (Hr_kq_monthsubmit_line) j;
			String sqlstr = "SELECT IFNULL(COUNT(*),0) ct FROM `Hr_kq_monthsubmit` h,  `Hr_kq_monthsubmit_line`  l"
