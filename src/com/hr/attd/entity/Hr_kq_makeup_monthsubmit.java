...
import com.hr.attd.ctr.CtrHr_kq_makeup_monthsubmit;
import com.hr.attd.ctr.CtrHr_kq_monthsubmit;

import java.sql.Types;

@CEntity(controller = CtrHr_kq_makeup_monthsubmit.class)
public class Hr_kq_makeup_monthsubmit extends CJPA {
	@CFieldinfo(fieldname ="mmkq_id",iskey=true,notnull=true,precision=20,scale=0,caption="补发出勤数据ID",datetype=Types.INTEGER)
	public CField mmkq_id;  //补发出勤数据ID
	@CFieldinfo(fieldname ="mmkq_code",codeid=148,notnull=true,precision=16,scale=0,caption="补发出勤数据编码",datetype=Types.VARCHAR)
	public CField mmkq_code;  //补发出勤数据编码
	...
	@CLinkFieldInfo(jpaclass = Hr_kq_makeup_monthsubmit_line.class, linkFields = { @LinkFieldItem(lfield = "mmkq_id", mfield = "mmkq_id") })
	public CJPALineData<Hr_kq_makeup_monthsubmit_line> hr_kq_makeup_monthsubmit_lines;
     //自关联数据定义


	public Hr_kq_makeup_monthsubmit() throws Exception {
	}
