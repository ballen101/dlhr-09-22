...
public class COreport1 {
	...
	public String findswcardlist() throws Exception {
		...
		String sqlstr = "SELECT e.er_id,e.employee_code,e.employee_name,e.card_number,e.orgid,e.orgcode,"
				+ " e.orgname,e.hg_name,e.lv_num,e.sp_name,p.machineno,p.machno1,p.skdate,p.sktype,e.idpath,e.emnature "// 增加人员性质
...
//					+ "tb.* from "
//					+ "(select "
//					+ "  e.er_id, e.employee_code, e.employee_name,e.empstatid, e.card_number, e.orgid, e.orgcode, e.orgname, e.hg_name, e.lv_num, e.sp_name, p.skdate "
//					+ "from "
//					+ "  (select * from hr_employee  where 1=1 ";
//			if (employee_code != null)
//				sqlstr = sqlstr + " and  employee_code='" + employee_code + "'";
//			sqlstr = sqlstr + " and noclock=2 and kqdate_start<'" + strbgdate + "' and ((empstatid in (1,2,3,4,5,7,8)) OR (empstatid>=10 and kqdate_end>='" + streddate + "'))"
//					+ CSContext.getIdpathwhere() + " ) e "
//					+ "  left join "
...
//					+ "where tb.skdate is null ";
			...
			String sqlstr = "SELECT  '" + streddate + "' cdate ,'" + Systemdate.getStrDateyyyy_mm_dd(tbgdate) + "到" + Systemdate.getStrDateyyyy_mm_dd(teddate) + "' memo, " +
					"  e.er_id, e.employee_code, e.employee_name,e.empstatid, e.card_number, e.orgid, e.orgcode, e.orgname, e.hg_name, e.lv_num, e.sp_name " +
					"FROM " +
					"  hr_employee e " +
					"WHERE  " +
					"  empstatid>0 ";
			if (employee_code != null)
				sqlstr = sqlstr + " and  employee_code='" + employee_code + "'";
			sqlstr += "  AND noclock = 2 " +
					"  AND kqdate_start < '" + strbgdate + "' " +
					"  AND ( " +
					"     kqdate_end > '" + streddate + "' " +
					"    OR  kqdate_end IS NULL " +
					"  ) " +
					...
					"  WHERE skdate >= '" + strbgdate + "' " +
					"    AND skdate <= '" + streddate + "' " +
					"    AND e.employee_code = c.empno)";

			String sqlstr2 = "SELECT er_id,GROUP_CONCAT(billtype) billtypes FROM ( " +
					"SELECT t.er_id, '请假' billtype FROM hrkq_holidayapp t  " +
					"WHERE t.stat>1 AND t.stat<10 AND t.timebg<'" + streddate + "' AND timeedtrue>='" + strbgdate + "' " +
					"UNION " +
					"SELECT t.er_id, '出差' billtype FROM hrkq_business_trip t  " +
					"WHERE t.stat>1 AND t.stat<10 AND t.begin_date<'" + streddate + "' AND end_date>='" + strbgdate + "' " +
					"UNION " +
					"SELECT t.er_id, '调休' billtype FROM hrkq_wkoff t  " +
					"WHERE t.stat>1 AND t.stat<10 AND t.begin_date<'" + streddate + "' AND end_date>='" + strbgdate + "') tb " +
					"GROUP BY tb.er_id";
			sqlstr = "select tb1.*,tb2.billtypes from (" + sqlstr + ") tb1 left join  (" + sqlstr2 + ") tb2  on tb1.er_id=tb2.er_id";
